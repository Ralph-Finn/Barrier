...
*******************************************************************************/
void	GPIOInterrupt( void ) interrupt INT_NO_TMR0  using 1                      //定时器中断服务程序
{ 
	///////////status light///////////////
	if(status_light == 0) MISO =1; 
	if(status_light == 1){
		nn++;
		if(nn==10){
			MISO = ~MISO;
	...
	if(status_light == 3) MISO =0;
	////////////power light/////////////
	if(power_light == 0) SCK =1; 
	if(power_light == 3) SCK =0;
}
#endif
